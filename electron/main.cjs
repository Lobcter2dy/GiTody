...
ipcMain.handle('get-disk-info', async () => {
  try {
    const [disks, fsSize, diskIO] = await Promise.all([si.diskLayout(), si.fsSize(), si.disksIO()]);
...
ipcMain.handle('ping-host', async (event, host, count = 4) => {
  return new Promise(resolve => {
    const cmd =
      process.platform === 'win32' ? `ping -n ${count} ${host}` : `ping -c ${count} ${host}`;
...
ipcMain.handle('get-disk-list', async () => {
  try {
    const [disks, fsSize] = await Promise.all([si.diskLayout(), si.fsSize()]);
...
function startOAuthCallbackServer() {
  oauthCallbackServer = http.createServer((req, res) => {
    ...
    if (url.pathname === '/callback') {
      ...
      if (error) {
        res.end(
          `<html><body><script>window.opener.postMessage({ type: 'github-oauth-error', error: '${error}' }, '*');window.close();</script></body></html>`
        );
      } else if (code) {
        res.end(
          `<html><body><script>window.opener.postMessage({ type: 'github-oauth-code', code: '${code}' }, '*');window.close();</script><p>Success!</p></body></html>`
        );
      } else {
